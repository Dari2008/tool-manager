...
export async function showDeleteModal({
  title = 'Delete',
  items = [] as DeletePlanItem[],
} = {}): Promise<string[]> {
  injectCss();

  return new Promise<string[]>(resolve => {
    const checkedMap = new Map<string, boolean>(items.map(i => [i.id, i.checked !== false]));
    const imgCardMap = new Map<string, { card: HTMLElement; cb: HTMLInputElement }>();

    const backdrop  = mk('div', 'dm-backdrop');
    const modal     = mk('div', 'dm-modal');
    const header    = mk('div', 'dm-header');
    const titleEl   = mk('div', 'dm-title'); titleEl.textContent = title;
    const closeBtn  = mk('button', 'dm-close') as HTMLButtonElement; closeBtn.textContent = '✕'; closeBtn.title = 'Cancel';
    ...
    const panelItems   = mk('div', 'dm-panel active'); panelItems.dataset.panel  = 'items';
    const panelImages  = mk('div', 'dm-panel');       panelImages.dataset.panel = 'images';
    ...
    function syncImg(id: string, val: boolean) {
      const e = imgCardMap.get(id); if (!e) return;
      e.cb.checked = val; e.card.classList.toggle('checked', val);
    }
    function syncRow(id: string, val: boolean) {
      const row = panelItems.querySelector<HTMLElement>(`.dm-row[data-id="${CSS.escape(id)}"]`);
      const cb  = row?.querySelector<HTMLInputElement>('.dm-row-cb'); if (cb) cb.checked = val;
    }

    // Items panel
    for (const item of items) {
      const row       = mk('div', 'dm-row'); row.dataset.id = item.id;
      const cb        = document.createElement('input');
      cb.type = 'checkbox'; cb.className = 'dm-row-cb'; cb.checked = checkedMap.get(item.id) ?? true;
      cb.addEventListener('change', () => { checkedMap.set(item.id, cb.checked); syncImg(item.id, cb.checked); });
      const info      = mk('div', 'dm-row-info');
      const typeEl    = mk('div', 'dm-row-type'); typeEl.textContent  = item.type;
      const labelEl   = mk('div', 'dm-row-label'); labelEl.textContent = item.label; labelEl.title = item.label;
      info.append(typeEl, labelEl);
      if (item.meta) { const m = mk('div', 'dm-row-meta'); m.textContent = item.meta; info.appendChild(m); }
      row.append(cb, info);
    ...
    if (!items.length) {
      ...
      for (const item of items) {
        ...
        cb.type = 'checkbox'; cb.className = 'dm-img-cb'; cb.checked = checkedMap.get(item.id) ?? true;
        cb.addEventListener('change', e => {
          e.stopPropagation(); checkedMap.set(item.id, cb.checked);
          card.classList.toggle('checked', cb.checked); syncRow(item.id, cb.checked);
        });
        if (item.imageUrl) {
          const img = document.createElement('img');
          img.src = item.imageUrl; img.alt = item.label; img.loading = 'lazy';
          ...
          const ic = mk('div', 'dm-img-placeholder-icon'); ic.textContent = '📄';
          const tl = mk('div', 'dm-img-placeholder-type'); tl.textContent = item.type;
          ph.append(ic, tl); card.appendChild(ph);
        }
        const lbl = mk('div', 'dm-img-label'); lbl.textContent = item.label;
        card.append(cb, lbl);
    ...
    [tabItems, tabImages].forEach(tab => {
      tab.addEventListener('click', () => {
        ...
        body.querySelectorAll('.dm-panel').forEach(p =>
          p.classList.toggle('active', (p as HTMLElement).dataset.panel === (tab as HTMLElement).dataset.tab)
        );
      });
    });

    selAllBtn.addEventListener('click', () => {
      items.forEach(i => { checkedMap.set(i.id, true); syncImg(i.id, true); });
      panelItems.querySelectorAll<HTMLInputElement>('.dm-row-cb').forEach(cb => { cb.checked = true; });
    });
    deselBtn.addEventListener('click', () => {
      items.forEach(i => { checkedMap.set(i.id, false); syncImg(i.id, false); });
      panelItems.querySelectorAll<HTMLInputElement>('.dm-row-cb').forEach(cb => { cb.checked = false; });
    });

    function doCancel() { backdrop.remove(); document.removeEventListener('keydown', onEsc); resolve([]); }
    function onEsc(e: KeyboardEvent) { if (e.key === 'Escape') doCancel(); }
    ...
    (confirmBtn as HTMLButtonElement).addEventListener('click', async () => {
      ...
      (closeBtn   as HTMLButtonElement).disabled    = true;
      const toDelete = items.filter(i => checkedMap.get(i.id));
      for (const item of toDelete) { try { await item.onDelete(); } catch { /**/ } }
      backdrop.remove(); document.removeEventListener('keydown', onEsc);
...
function mkTab(label: string, key: string, active: boolean): HTMLElement {
  const btn = mk('button', 'dm-tab' + (active ? ' active' : ''));
  btn.textContent        = label;
  (btn as HTMLElement & { dataset: DOMStringMap }).dataset.tab = key;
  return btn;
}

function mkBtn(label: string, extra = ''): HTMLElement {
